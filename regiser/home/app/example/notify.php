...
class PayNotifyCallBack extends WxPayNotify
{
	...
	public function NotifyProcess($data, &$msg)
	{
		...
		if(!$this->Queryorder($data["transaction_id"])){
			...
			$check=$db->table("marathon")->where(array('transaction_id'=>$data['transaction_id']))->find();
			$recharge=$db->table("marathon")->where(array('out_trade_no'=>$data['out_trade_no']))->find();
			if(!$check){
				$mydata['ispay']=1;
				$result=$db->table("marathon")->data($mydata)->where($where)->update();
				Log::DEBUG("notifySql-->".$db->getSql());
				if($result){
					Log::DEBUG("成功支付".$data['total_fee']."分");
				}else{
					Log::DEBUG("-->支付失败，请联系他");
				}
